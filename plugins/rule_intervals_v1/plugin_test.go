...
import (
	...
	plugin "github.com/slok/sloth-test-slo-plugins/rule_intervals_v1"
)
	...
	tests := map[string]struct {
		...
		"A config without default time should fail.": {
			config: MustJSONRawMessage(t, plugin.Config{
				Interval: plugin.ConfigInterval{
...
func BenchmarkPluginYaegi(b *testing.B) {
	plugin, err := pluginslov1testing.NewTestPlugin(b.Context(), pluginslov1testing.TestPluginConfig{PluginConfiguration: []byte(`{"interval": {"default": "5m"}}`)})
	if err != nil {
		b.Fatal(err)
	}
...
func BenchmarkPluginGo(b *testing.B) {
	plugin, err := plugin.NewPlugin([]byte(`{"interval": {"default": "5m"}}`), pluginslov1.AppUtils{})
	if err != nil {
		b.Fatal(err)
	}
